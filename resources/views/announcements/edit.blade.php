<x-app-layout>
    <x-slot name="header">
        <h2 class="font-display font-semibold text-xl text-ink">Edit Announcement</h2>
    </x-slot>
    <div class="py-8">
            ...
            <div class="bg-white rounded-xl border border-sand-200 p-6">
                <form method="POST" action="{{ route('announcements.update', $announcement) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="title" value="{{ old('title', $announcement->title) }}" class="w-full border-sand-200 rounded-lg focus:border-teal-700 focus:ring-teal-700" required>
                    </div>
                    <div class="mb-4">
                        ...
                        <select name="type" class="w-full border-sand-200 rounded-lg focus:border-teal-700 focus:ring-teal-700" required>
                            ...
                            <option value="burial" {{ old('type', $announcement->type) === 'burial' ? 'selected' : '' }}>Burial Notice</option>
                        </select>
                    </div>
                    <div class="mb-4">
                        <label class="block font-medium text-sm text-ink mb-1">Body</label>
                        <textarea name="body" class="w-full border-sand-200 rounded-lg focus:border-teal-700 focus:ring-teal-700" rows="5" required>{{ old('body', $announcement->body) }}</textarea>
                    </div>
                    <div class="mb-4">
                        <label class="block font-medium text-sm text-ink mb-1">Publish Date</label>
                        <input type="datetime-local" name="published_at" value="{{ old('published_at', $announcement->published_at) }}" class="w-full border-sand-200 rounded-lg focus:border-teal-700 focus:ring-teal-700">
                    </div>
                    <div class="mb-6 flex items-center">
                        <input type="checkbox" name="is_pinned" value="1" id="is_pinned" class="mr-2 rounded border-sand-300 text-gold-600 focus:ring-gold-600" {{ $announcement->is_pinned ? 'checked' : '' }}>
