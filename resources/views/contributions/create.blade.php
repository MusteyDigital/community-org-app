<x-app-layout>
    <x-slot name="header">
        <h2 class="font-display font-semibold text-xl text-ink">Record Contribution</h2>
    </x-slot>
    <div class="py-8">
            ...
            <div class="bg-white rounded-xl border border-sand-200 p-6">
                <form method="POST" action="{{ route('contributions.store') }}">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" step="0.01" min="0.01" name="amount" value="{{ old('amount') }}" class="w-full border-sand-200 rounded-lg focus:border-teal-700 focus:ring-teal-700" required>
                    </div>
                    <div class="mb-4">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="date" name="contributed_at" value="{{ old('contributed_at', date('Y-m-d')) }}" class="w-full border-sand-200 rounded-lg focus:border-teal-700 focus:ring-teal-700" required>
                    </div>
                    <div class="mb-6">
                        <label class="block font-medium text-sm text-ink mb-1">Note (optional)</label>
                        <textarea name="note" class="w-full border-sand-200 rounded-lg focus:border-teal-700 focus:ring-teal-700" rows="3">{{ old('note') }}</textarea>
                    </div>
